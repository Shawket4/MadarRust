-- Hunt H2-B8 (SC-5, RO-6): a business-wide block on a two-branch person's
-- date was always worked at their FIRST branch, so one put on the day from
-- branch B's board landed on A's roster and vanished from B's. A date's own
-- row now records where a business-wide block is worked (the board it was
-- set from, or where a claimed, moved or swapped shift was worked). NULL =
-- as before: the block's branch, else the person's first. A branch's own
-- block always counts at that branch, so the column is only written for a
-- business-wide block, and only with one of the person's branches.
ALTER TABLE staff_schedule_overrides
    ADD COLUMN branch_id uuid REFERENCES branches(id) ON DELETE SET NULL;
COMMENT ON COLUMN staff_schedule_overrides.branch_id IS
    'Where a business-wide block is worked that date (one of the person''s branches); NULL = the block''s branch, else the person''s first.';

-- The one roster function (as 20260930100000), with the date's own branch
-- between the block's and the person's first.
