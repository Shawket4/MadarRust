...
pub const PRESETS: &[Preset] = &[
    ...
    preset!("revenue_total", "Revenue", "Sales", "reports", "orders",
        ...
        "Total revenue for the period: order totals after discount, including tax, service charge and delivery fees, net of any refunds against those orders. Excludes voided and fully refunded orders."),
    preset!("order_count_total", "Orders", "Sales", "reports", "orders",
        dims: [], measures: ["order_count"], filters: [],
        sort: None, limit: 1, viz: Viz::Kpi, period: PeriodPreset::Today, share: false,
        "Number of orders taken in the period, excluding voided and fully refunded ones."),
    preset!("avg_ticket", "Average ticket", "Sales", "reports", "orders",
        dims: [], measures: ["avg_order_value"], filters: [],
        sort: None, limit: 1, viz: Viz::Kpi, period: PeriodPreset::Today, share: false,
        "Average order total as rung up — the average amount a customer spends per visit."),
    preset!("sales_summary", "Sales summary", "Sales", "reports", "orders",
        dims: [], measures: ["order_count", "gross_sales", "refund_amount", "revenue", "avg_order_value", "discount_total", "service_charge_total", "tip_total"],
        filters: [], sort: None, limit: 1, viz: Viz::Table, period: PeriodPreset::Today, share: false,
        "Headline sales figures for the period: orders, sales as rung up, refunds against them, revenue kept, average ticket, discounts given, service charge added and tips collected."),
    preset!("service_charge_total", "Service charge", "Sales", "reports", "orders",
        dims: [], measures: ["service_charge_total", "order_count"], filters: [("order_type", "dine_in")],
        sort: None, limit: 1, viz: Viz::Kpi, period: PeriodPreset::Today, share: false,
        "Service charge added to dine-in bills in the period. Counted as the shop's income inside revenue; shown here on its own."),
    ...
    preset!("sales_by_order_type", "Dine-in, takeaway, delivery", "Sales", "reports", "orders",
        dims: ["order_type"], measures: ["revenue", "order_count", "avg_order_value", "service_charge_total"], filters: [],
        sort: Some(("revenue", Dir::Desc)), limit: 10, viz: Viz::Donut, period: PeriodPreset::Last30Days, share: true,
        "Revenue split between dine-in, takeaway and delivery orders, with the service charge only dine-in carries."),
    preset!("sales_by_channel", "Delivery channels", "Sales", "reports", "orders",
        dims: ["delivery_channel"], measures: ["revenue", "delivery_fees", "net_revenue", "order_count"],
        filters: [("order_type", "delivery")], sort: Some(("revenue", Dir::Desc)), limit: 10,
        viz: Viz::Donut, period: PeriodPreset::Last30Days, share: true,
        "Delivery revenue by channel: in-mall, outside, umbrella aggregators and pickup. The delivery fee is shown apart — it is outside the tax base and is not food revenue — and net revenue excludes it."),
    // ── Tips ─────────────────────────────────────────────────────────────────
    ...
    // ── Refunds ──────────────────────────────────────────────────────────────
    // On the refunds dataset, by the day the money went back — not the orders
    // dataset filtered to status 'refunded', which only ever saw FULL refunds.
    preset!("refunds_by_day", "Refunds", "Control", "reports", "refunds",
        dims: ["day"], measures: ["refund_amount", "cash_refund_amount", "refund_count"],
        filters: [], sort: None, limit: 90, viz: Viz::Line,
        period: PeriodPreset::Last30Days, share: false,
        "Money refunded per day — all tenders and the cash slice that left a drawer — by the day each refund was issued."),
    preset!("refunds_by_reason", "Refunds by reason", "Control", "reports", "refunds",
        dims: ["refund_reason"], measures: ["refund_amount", "refund_count", "orders_refunded"],
        filters: [], sort: Some(("refund_amount", Dir::Desc)), limit: 10, viz: Viz::Bar,
        period: PeriodPreset::Last30Days, share: false,
        "Refunds grouped by the reason given, with the value returned. Wrong order and quality issue share their spelling with the void reasons."),
    preset!("refunds_by_cashier", "Refunds by cashier", "Control", "reports", "refunds",
        dims: ["cashier"], measures: ["refund_amount", "refund_count"],
        filters: [], sort: Some(("refund_amount", Dir::Desc)), limit: 30, viz: Viz::Bar,
        period: PeriodPreset::Last30Days, share: false,
        "Who issues refunds, by count and value — the same loss-prevention check as voids by cashier."),
    // ── Products ─────────────────────────────────────────────────────────────
