...
async fn fixture(pool: &PgPool) -> Fx {
    ...
    sqlx::query("INSERT INTO devices (id, org_id, branch_id, code) VALUES ($1, $2, $3, '36B')")
        .bind(device).bind(org).bind(branch).execute(pool).await.unwrap();
    Fx {
        org,
        branch,
...
fn list(ids: &[Uuid]) -> AllowList {
    AllowList { restricted: true, payment_method_ids: ids.to_vec() }
}

async fn names(pool: &PgPool, fx: &Fx, user: Option<Uuid>, device: Option<Uuid>) -> Vec<String> {
    effective_method_names(pool, fx.org, fx.branch, user, device).await.unwrap()
}
...
async fn no_rows_means_unrestricted(pool: PgPool) {
    ...
    assert_eq!(names(&pool, &fx, Some(fx.teller), Some(fx.device)).await, all);
    assert!(!is_method_available(&pool, fx.org, fx.branch, None, None, "old").await.unwrap());
    assert!(!is_method_available(&pool, fx.org, fx.branch, None, None, "nope").await.unwrap());
...
async fn effective_methods_intersection_branch_user_device(pool: PgPool) {
    let fx = fixture(&pool).await;
    replace_list(&pool, fx.org, Owner::Branch, fx.branch, &list(&[fx.cash, fx.card, fx.cib, fx.inactive])).await.unwrap();
    replace_list(&pool, fx.org, Owner::User, fx.teller, &list(&[fx.cash, fx.cib, fx.card])).await.unwrap();
    replace_list(&pool, fx.org, Owner::Device, fx.device, &list(&[fx.cash, fx.cib])).await.unwrap();

    assert_eq!(names(&pool, &fx, None, None).await, ["cash", "card", "CIB – counter"]); // inactive never
    assert_eq!(names(&pool, &fx, Some(fx.teller), Some(fx.device)).await, ["cash", "CIB – counter"]);
    replace_list(&pool, fx.org, Owner::User, fx.teller, &list(&[fx.card, fx.cash])).await.unwrap();
    assert_eq!(names(&pool, &fx, Some(fx.teller), Some(fx.device)).await, ["cash"]);
    assert_eq!(names(&pool, &fx, Some(fx.teller), None).await, ["cash", "card"]);
    assert!(!is_method_available(&pool, fx.org, fx.branch, Some(fx.teller), Some(fx.device), "card").await.unwrap());
    assert!(is_method_available(&pool, fx.org, fx.branch, Some(fx.teller), None, "card").await.unwrap());
    // Another teller with no rows is unrestricted by user.
    let other = user(&pool, fx.org, "teller").await;
    assert_eq!(names(&pool, &fx, Some(other), Some(fx.device)).await, ["cash", "CIB – counter"]);
    ...
    let off = AllowList { restricted: false, payment_method_ids: vec![] };
    for _ in 0..2 {
        let r = replace_list(&pool, fx.org, Owner::Device, fx.device, &off).await.unwrap();
        assert_eq!(r, off);
    }
    assert_eq!(names(&pool, &fx, Some(fx.teller), Some(fx.device)).await, ["cash", "card"]);
}
...
async fn cross_org_ids_rejected(pool: PgPool) {
    ...
    let e = replace_list(&pool, fx.org, Owner::Branch, fx.branch, &list(&[other.card])).await.unwrap_err();
    assert!(matches!(e, crate::errors::AppError::BadRequest(_)));
    let e = replace_list(&pool, fx.org, Owner::User, other.teller, &list(&[fx.card])).await.unwrap_err();
...
async fn put_restricted_empty_is_400_and_permissions(pool: PgPool) {
    ...
    let r = test::call_service(&app, put(&admin, format!("/payment-methods/availability/branches/{}", fx.branch),
        serde_json::json!({"restricted": true, "payment_method_ids": []}))).await;
    assert_eq!(r.status(), StatusCode::BAD_REQUEST);
    let body: serde_json::Value = test::read_body_json(r).await;
    assert!(body.to_string().contains(CODE_EMPTY_ALLOW_LIST));

    let r = test::call_service(&app, put(&teller, format!("/payment-methods/availability/users/{}", fx.teller),
        serde_json::json!({"restricted": true, "payment_method_ids": [fx.cash]}))).await;
    assert_eq!(r.status(), StatusCode::FORBIDDEN);

    let r = test::call_service(&app, put(&admin, format!("/payment-methods/availability/devices/{}", fx.device),
        serde_json::json!({"restricted": true, "payment_method_ids": [fx.cib, fx.cib]}))).await;
    ...
    let get = |uri: String| test::TestRequest::get().uri(&uri)
        .insert_header(("Authorization", format!("Bearer {teller}"))).to_request();
    let r = test::call_service(&app, get(format!("/payment-methods/effective?branch_id={}&device_id={}", fx.branch, fx.device))).await;
    assert_eq!(r.status(), StatusCode::OK);
    let v: serde_json::Value = test::read_body_json(r).await;
    assert_eq!(v.as_array().unwrap().len(), 1);
    let r = test::call_service(&app, get(format!("/payment-methods/availability?branch_id={}", fx.branch))).await;
    assert_eq!(r.status(), StatusCode::OK);
