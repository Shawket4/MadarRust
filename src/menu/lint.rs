...
//!
//! The swap vocabulary is today's inference (`component_resolve.rs`): a group whose
//! `legacy_addon_type` is `milk_type` / `coffee_type` swaps the recipe line whose
//! ingredient category slug is `milk` / `coffee_bean`.
//!
...
type Row = (Uuid, String, Option<String>, String);
    ...
    () => {
        "swap AS (
            SELECT g.id, g.name, g.legacy_addon_type,
                   CASE g.legacy_addon_type WHEN 'milk_type' THEN 'milk' ELSE 'coffee_bean' END AS slug
...
const RULES: &[Rule] = &[
    ...
    Rule {
        ...
        sql: concat!(
            ...
            SELECT DISTINCT item_id, item_name, size_label, msg FROM swaps
            UNION
            SELECT DISTINCT item_id, item_name, size_label, msg FROM follows
             ORDER BY 2, 3, 4"
        ),
    },
    Rule {
...
pub async fn lint_org(pool: &PgPool, org_id: Uuid) -> Result<Vec<LintIssue>, AppError> {
    ...
    for rule in RULES {
        ...
        out.extend(
            rows.into_iter()
                .map(|(entity_id, entity_name, size_label, message)| LintIssue {
...
pub async fn get_menu_lint(
    ...
    let issues = lint_org(pool.get_ref(), query.org_id).await?;
