...
pub fn configure(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/shifts")
            ...
            .route("/{shift_id}/close", web::post().to(handlers::close_shift))
            .route(
                "/{shift_id}/force-close",
                web::post().to(handlers::force_close_shift),
            )
            .route("/{shift_id}", web::get().to(handlers::get_shift))
            .route("/{shift_id}", web::delete().to(handlers::delete_shift)),
    );
}
