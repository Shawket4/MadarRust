...
async fn test_e2e_menu_advisor_bundle_promotion_workflow(pool: PgPool) {
    ...
    let org_id = Uuid::new_v4();
    sqlx::query(
        "INSERT INTO organizations (id, name, slug) VALUES ($1, 'Advisor E2E Org', 'adv-org')",
    )
    .bind(org_id)
    .execute(&pool)
    .await
    ...
    let coffee_item: MenuItemFull = test::read_body_json(
        test::call_service(
            &app,
            test::TestRequest::post()
                .uri("/menu-items")
    ...
    let croissant_item: MenuItemFull = test::read_body_json(
        test::call_service(
            &app,
            test::TestRequest::post()
                .uri("/menu-items")
    ...
    let beans: OrgIngredient = test::read_body_json(
        test::call_service(
            &app,
            test::TestRequest::post()
                .uri(&format!("/inventory/orgs/{}/catalog", org_id))
    ...
    let dough: OrgIngredient = test::read_body_json(
        test::call_service(
            &app,
            test::TestRequest::post()
                .uri(&format!("/inventory/orgs/{}/catalog", org_id))
    ...
    let _stock_dough: BranchInventoryItem = test::read_body_json(
        test::call_service(
            &app,
            test::TestRequest::post()
                .uri(&format!("/inventory/branches/{}/stock", branch_id))
    ...
    let resp = test::call_service(
        &app,
        test::TestRequest::post()
            .uri(&format!("/recipes/drinks/{}", coffee_id))
            .insert_header(("Authorization", format!("Bearer {}", admin_token)))
            .set_json(&serde_json::json!({
                "size_label": "one_size",
                "org_ingredient_id": beans.id,
    ...
    let run_id: Uuid = sqlx::query_scalar(
        "SELECT id FROM menu_advisor_runs WHERE branch_id = $1 ORDER BY started_at DESC LIMIT 1",
    )
    .bind(branch_id)
    .fetch_one(&pool)
    .await
    ...
    sqlx::query(
        "UPDATE menu_advisor_runs SET status = 'completed', completed_at = NOW() WHERE id = $1",
    )
    .bind(run_id)
    .execute(&pool)
    .await
    ...
    let price_payload = serde_json::to_value(adv::PriceSuggestion {
        ...
        classification: adv::Classification::Cm {
            quadrant: adv::CmQuadrant::Star,
        },
        current_price: 350,
        units_sold_raw: 10.0,
    ...
    let bundle_payload = serde_json::to_value(adv::BundleSuggestion {
        focus_item: adv::ItemKey {
            menu_item_id: coffee_id,
            size_label: "one_size".into(),
        },
        bundle_items: vec![adv::ItemKey {
            menu_item_id: coffee_id,
            size_label: "one_size".into(),
        }],
        ...
        association: adv::BundleAssociation {
            pair_lifts: vec![],
            composite_score: 1.2,
        },
        forecast: adv::BundleForecast {
            expected_velocity: adv::Triplet {
                lo: 5.0,
                mid: 10.0,
                hi: 15.0,
            },
    ...
    let dough_stock: sqlx::types::BigDecimal = sqlx::query_scalar(
        "SELECT current_stock FROM branch_inventory WHERE org_ingredient_id = $1",
    )
    .bind(dough.id)
    .fetch_one(&pool)
    .await
