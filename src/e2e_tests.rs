...
async fn test_e2e_purchasing_stocktake_reporting_lifecycle(pool: PgPool) {
    ...
    seed_payment_methods(&pool, org_id).await;
    let branch_id = Uuid::new_v4();
    sqlx::query("INSERT INTO branches (id, org_id, name) VALUES ($1,$2,'Main')").bind(branch_id).bind(org_id).execute(&pool).await.unwrap();
    ...
    sqlx::query("INSERT INTO user_branch_assignments (user_id, branch_id) VALUES ($1,$2)").bind(teller_id).bind(branch_id).execute(&pool).await.unwrap();
    ...
    // ── A. Supplier ──
    let (st, v) = jget!(app, test::TestRequest::post()
        .uri(&format!("/purchasing/orgs/{org_id}/suppliers")).insert_header(("Authorization", abearer.clone()))
        .set_json(serde_json::json!({"name": "Beans Co", "phone": "0100"})).to_request());
    assert_eq!(st, 201);
    ...
    let (st, recv) = jget!(app, test::TestRequest::post()
        .uri(&format!("/purchasing/orders/{beans_po}/receive")).insert_header(("Authorization", abearer.clone()))
    ...
    let (_, r1) = jget!(app, test::TestRequest::post()
        .uri(&format!("/purchasing/orders/{milk_po}/receive")).insert_header(("Authorization", abearer.clone()))
        .set_json(serde_json::json!({"lines":[{"line_id":milk_line,"quantity_received":1.0}]})).to_request());
    assert_eq!(r1["status"], "partially_received");
    // Receive the rest → received.
    let (_, r2) = jget!(app, test::TestRequest::post()
        .uri(&format!("/purchasing/orders/{milk_po}/receive")).insert_header(("Authorization", abearer.clone()))
        .set_json(serde_json::json!({"lines":[{"line_id":milk_line,"quantity_received":1.0}]})).to_request());
    ...
    let (_, cat) = jget!(app, test::TestRequest::post()
        .uri("/categories").insert_header(("Authorization", abearer.clone()))
        .set_json(serde_json::json!({"org_id":org_id,"name":"Coffee"})).to_request());
    ...
    assert_eq!(st, 200);
    let (_, shift) = jget!(app, test::TestRequest::post()
        .uri(&format!("/shifts/branches/{branch_id}/open")).insert_header(("Authorization", tbearer.clone()))
        .set_json(serde_json::json!({"opening_cash":0})).to_request());
    ...
    // ── G. Waste milk ──
    let st = jstatus!(app, test::TestRequest::post()
        .uri(&format!("/inventory/branches/{branch_id}/waste")).insert_header(("Authorization", abearer.clone()))
        .set_json(serde_json::json!({"org_ingredient_id":milk,"quantity":100.0,"reason":"spoiled"})).to_request());
    assert_eq!(st, 201);

    // ── H. Stock count with the variance guardrail ──
    let (st, full) = jget!(app, test::TestRequest::post()
        .uri(&format!("/stocktakes/branches/{branch_id}")).insert_header(("Authorization", abearer.clone()))
    ...
    let st = jstatus!(app, test::TestRequest::put()
        ...
        .set_json(serde_json::json!({"items":[{"org_ingredient_id":beans,"counted_qty":1500.0}]})).to_request());
    assert_eq!(st, 200);
    // Finalize blocked until a reason is given.
    let st = jstatus!(app, test::TestRequest::post()
        .uri(&format!("/stocktakes/{stocktake_id}/finalize")).insert_header(("Authorization", abearer.clone())).to_request());
    assert_eq!(st, 409);
    ...
    let (_, val) = jget!(app, test::TestRequest::get()
        .uri(&format!("/reports/orgs/{org_id}/inventory-valuation")).insert_header(("Authorization", abearer.clone())).to_request());
    ...
    let (_, low) = jget!(app, test::TestRequest::get()
        .uri(&format!("/reports/orgs/{org_id}/low-stock")).insert_header(("Authorization", abearer.clone())).to_request());
    let milk_low = low.as_array().unwrap().iter().find(|r| r["org_ingredient_id"] == serde_json::json!(milk)).expect("milk should be low");
    assert_eq!(milk_low["supplier_name"], "Beans Co");

    // Consumption (branch): beans 36 (sale), milk 100 (waste).
    let (_, cons) = jget!(app, test::TestRequest::get()
        .uri(&format!("/reports/branches/{branch_id}/consumption")).insert_header(("Authorization", abearer.clone())).to_request());
    let beans_cons = cons.as_array().unwrap().iter().find(|r| r["org_ingredient_id"] == serde_json::json!(beans)).unwrap();
    assert_eq!(beans_cons["consumed_qty"].as_f64().unwrap(), 36.0);
    let milk_cons = cons.as_array().unwrap().iter().find(|r| r["org_ingredient_id"] == serde_json::json!(milk)).unwrap();
    assert_eq!(milk_cons["consumed_qty"].as_f64().unwrap(), 100.0);

    // Shrinkage (branch): beans, reason 'theft', 464 lost.
    let (_, shr) = jget!(app, test::TestRequest::get()
        .uri(&format!("/reports/branches/{branch_id}/shrinkage")).insert_header(("Authorization", abearer.clone())).to_request());
    let beans_shr = shr.as_array().unwrap().iter().find(|r| r["org_ingredient_id"] == serde_json::json!(beans)).unwrap();
    ...
    let (_, wr) = jget!(app, test::TestRequest::get()
        .uri(&format!("/reports/branches/{branch_id}/waste-report")).insert_header(("Authorization", abearer.clone())).to_request());
    let milk_waste = wr.as_array().unwrap().iter().find(|r| r["org_ingredient_id"] == serde_json::json!(milk)).unwrap();
