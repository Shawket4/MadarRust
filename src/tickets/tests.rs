...
async fn seed_user(pool: &PgPool, org: Uuid, role: &str) -> Uuid {
    ...
    .bind(id).bind(org).bind(format!("{role}-{id}")).bind(format!("{id}@t.com")).bind(role)
    .execute(pool).await.unwrap();
    id
}
async fn seed_menu_item(pool: &PgPool, org: Uuid, price: i32) -> Uuid {
    let id = Uuid::new_v4();
    sqlx::query("INSERT INTO menu_items (id, org_id, name, base_price) VALUES ($1, $2, 'Burger', $3)")
        .bind(id).bind(org).bind(price).execute(pool).await.unwrap();
    id
}
...
async fn waiter_fire_bump_settle_end_to_end(pool: PgPool) {
    ...
    assert_eq!(view.opened_by, waiter, "waiter is preserved as the order-taker");
    ...
    let feed_resp = test::call_service(&app, test::TestRequest::get()
    ...
    let bump = test::call_service(&app, test::TestRequest::post()
    ...
    let settle = test::call_service(&app, test::TestRequest::post()
    ...
    assert_eq!(order.shift_id, shift, "lands in the settling cashier's shift");
...
async fn replay_fire_round_settle_idempotent_and_attributed(pool: PgPool) {
    ...
    // each op's EMBEDDED actor, so no per-op permission grants are needed.
    ...
    let resp = test::call_service(&app, test::TestRequest::post()
        ...
        .set_json(&fire).to_request()).await;
    assert_eq!(resp.status(), 201, "replayed fire creates the ticket");
    let view: OpenTicketView = test::read_body_json(resp).await;
    let ticket_id = view.id;
    assert_eq!(view.opened_by, waiter, "attributed to the embedded waiter, not the bearer");
    assert_eq!(view.subtotal, 2000);
    ...
    let n: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM open_tickets WHERE idempotency_key=$1")
        .bind(ticket_idem).fetch_one(&pool).await.unwrap();
    ...
    for _ in 0..2 {
        let r = test::call_service(&app, test::TestRequest::post()
            .uri("/sync/replay")
            .insert_header(("Authorization", format!("Bearer {bearer}")))
            .set_json(&round).to_request()).await;
        assert!(r.status().is_success(), "replayed round ok");
    }
    let rounds: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM open_ticket_rounds WHERE open_ticket_id=$1")
        .bind(ticket_id).fetch_one(&pool).await.unwrap();
    ...
    let s1 = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&settle).to_request()).await;
    ...
    let s2 = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&settle).to_request()).await;
    assert_eq!(s2.status(), 200, "replayed settle is idempotent (lost ack), not a 409");
...
async fn replay_rejects_wrong_actor_role_or_org(pool: PgPool) {
    ...
    let r = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&fire_by_teller).to_request()).await;
    ...
    let r = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&settle_by_waiter).to_request()).await;
    ...
    let r = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&cross).to_request()).await;
...
async fn fire_derives_stable_kitchen_ids_from_round_key(pool: PgPool) {
    ...
    assert_eq!(got_item, crate::kitchen::derive_kitchen_item_id(kt, 0), "line 0 id derived");
}
...
async fn replay_bump_idempotent_and_attributed(pool: PgPool) {
    ...
    grant(&pool, "waiter", "open_tickets", "create").await;
    let waiter_t = token(waiter, org, UserRole::Waiter);
    // The flushing device signs in as a teller; replay authorizes via each op's
    // embedded actor, so the bump lands under the kitchen user with no grants.
    let bearer = token(teller, org, UserRole::Teller);
    ...
    let r = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&bump).to_request()).await;
    ...
    let (status, bumped_by): (String, Option<Uuid>) = sqlx::query_as(
        ...
        .bind(ticket_id).bind(kitchen_item_id).fetch_one(&pool).await.unwrap();
    assert_eq!(status, "ready", "only line bumped → ticket ready");
    assert_eq!(bumped_by, Some(kitchen), "attributed to the embedded kitchen actor, not the bearer");
    ...
    let rg = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&ghost).to_request()).await;
    assert_eq!(rg.status(), 204, "bump for a gone line is a no-op, not an error");
    ...
    let rw = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&bump_by_waiter).to_request()).await;
    ...
    let ru = test::call_service(&app, test::TestRequest::post()
        .uri("/sync/replay")
        .insert_header(("Authorization", format!("Bearer {bearer}")))
        .set_json(&unbump).to_request()).await;
...
async fn replay_fire_publishes_realtime(pool: PgPool) {
    ...
    let resp = test::call_service(&app, test::TestRequest::post()
        ...
        .insert_header(("Authorization", format!("Bearer {waiter_t}")))
        .set_json(&fire).to_request()).await;
    assert!(resp.status().is_success(), "replayed fire ok: {}", resp.status());
