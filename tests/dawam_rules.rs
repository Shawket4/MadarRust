...
async fn seed(pool: &PgPool) -> F {
    ...
    let e = employee(pool, org, "Eman", None, Some("+201060000001"), true, &[a], SALARY).await;
    let e_mgr = employee(pool, org, "Mona", Some(mgr), Some("+201060000002"), true, &[a], SALARY).await;
    let e_peer = employee(pool, org, "Peter", Some(peer), Some("+201060000003"), true, &[a], SALARY).await;
    let e_owner = employee(pool, org, "Omar", Some(owner), Some("+201060000004"), true, &[a], SALARY).await;
    let x = employee(pool, org, "Xavier", None, Some("+201060000005"), true, &[b], SALARY).await;
...
fn at(date: &str, hm: &str) -> DateTime<Utc> {
    ...
    Utc.from_utc_datetime(&d.and_hms_opt(h.parse().unwrap(), m.parse().unwrap(), 0).unwrap())
}
...
where
    S: actix_web::dev::Service<
        ...
        >,
{
    let (st, b) = send!(*app, "PATCH", format!("/staff/attendance/{rec}"), f.owner_token(), json!({ "reason": "re-derive" }));
    assert_eq!(st, 200, "{b}");
}
...
async fn file(
    ...
    let (st, row) = send!(*app, "POST", "/staff/requests".to_string(), f.owner_token(), b);
...
async fn unpaid_leave_without_a_type_docks_a_day_and_paid_leave_does_not(pool: PgPool) {
    ...
    assert_eq!(deduction(&pool, rec, "absence").await, DAY, "absent: one day");

    let leave = file(&app, &f, f.e, json!({ "kind": "leave", "on_date": d })).await;
    let (st, row) = decide(&app, &f.owner_token(), &leave["id"], json!({ "status": "approved", "is_paid": false })).await;
    ...
    let (st, _) = decide(&app, &f.owner_token(), &leave["id"], json!({ "status": "cancelled", "note": "wrong day" })).await;
    assert_eq!(st, 200);
    let paid = file(&app, &f, f.e, json!({ "kind": "leave", "on_date": d })).await;
    let (st, _) = decide(&app, &f.owner_token(), &paid["id"], json!({ "status": "approved", "is_paid": true })).await;
    assert_eq!(st, 200);
    assert_eq!(deduction(&pool, rec, "absence").await, 0, "paid leave costs nothing");
}
...
async fn an_approved_month_is_closed_to_requests_and_manual_edits(pool: PgPool) {
    ...
    let f = seed(&pool).await;
    let d = "2026-08-10";
    let rec = record(&pool, &f, f.e, f.day_shift, d, (at(d, "09:00"), at(d, "17:00")), Some((at(d, "09:00"), at(d, "17:00"))), "present").await;
    let pending = file(&app, &f, f.e, json!({ "kind": "leave", "on_date": "2026-08-12" })).await;
    let approved = file(&app, &f, f.e, json!({ "kind": "late_arrival", "on_date": "2026-08-13", "to_time": "10:00:00" })).await;
    let (st, _) = decide(&app, &f.owner_token(), &approved["id"], json!({ "status": "approved" })).await;
    assert_eq!(st, 200);
    ...
    let (st, _) = decide(&app, &f.owner_token(), &pending["id"], json!({ "status": "approved", "is_paid": true })).await;
    assert_eq!(st, 409, "approving into an approved month");
    let (st, _) = decide(&app, &f.owner_token(), &approved["id"], json!({ "status": "cancelled", "note": "n" })).await;
    assert_eq!(st, 409, "cancelling approved time after payroll approval (RQ-12)");
    let (st, _) = decide(&app, &f.owner_token(), &pending["id"], json!({ "status": "rejected" })).await;
    assert_eq!(st, 200, "rejecting changes nothing that was paid");

    // AT-7: manual attendance edits wait for next month too.
    let (st, _) = send!(app, "PATCH", format!("/staff/attendance/{rec}"), f.owner_token(), json!({ "reason": "fix", "status": "absent" }));
    assert_eq!(st, 409);
    let (st, _) = send!(app, "POST", "/staff/attendance".to_string(), f.owner_token(), json!({ "employee_id": f.e, "branch_id": f.a, "business_date": "2026-08-11", "status": "absent", "reason": "r" }));
    assert_eq!(st, 409);
    let (st, _) = send!(app, "DELETE", format!("/staff/attendance/{rec}"), f.owner_token());
    assert_eq!(st, 409);
    // September is still open.
    let (st, _) = send!(app, "POST", "/staff/requests".to_string(), f.owner_token(), json!({ "employee_id": f.e, "kind": "leave", "on_date": "2026-09-02" }));
    assert_eq!(st, 201);
}
...
async fn the_owners_own_request_is_approved_as_it_is_filed(pool: PgPool) {
    ...
    let (st, row) = send!(app, "POST", "/staff/me/requests".to_string(), phone, json!({ "kind": "late_arrival", "on_date": "2026-09-10", "to_time": "10:00:00" }));
    assert_eq!(st, 201, "{row}");
    assert_eq!(row["status"], "approved", "hr.requests.self_approve: approved at filing (RQ-5, B12)");
...
async fn a_managers_own_request_goes_to_the_owner_not_to_a_peer(pool: PgPool) {
    ...
    let phone = format!("{}|{}", s.token, s.device);
    let (st, row) = send!(app, "POST", "/staff/me/requests".to_string(), phone.clone(), json!({ "kind": "early_departure", "on_date": "2026-09-10", "from_time": "16:00:00", "reason": "." }));
    assert_eq!(st, 201, "{row}");
    assert_eq!(row["status"], "pending", "a manager without self_approve waits");
    assert_eq!(row["to_owner"], true, "flagged to the owner");
    assert!(row["reason"].is_null(), "a note of only punctuation is no note: {row}");
    ...
    let (st, _) = decide(&app, &f.mgr_token(), &row["id"], json!({ "status": "approved" })).await;
    ...
    let (st, b) = decide(&app, &f.peer_token(), &row["id"], json!({ "status": "approved" })).await;
    assert_eq!(st, 403, "a peer decides a peer's request: {b}");
    // The owner decides it.
    let (st, b) = decide(&app, &f.owner_token(), &row["id"], json!({ "status": "approved" })).await;
    assert_eq!(st, 200, "{b}");
    ...
    let (st, _) = decide(&app, &phone, &row["id"], json!({ "status": "cancelled", "note": "plans changed" })).await;
    assert_eq!(st, 200);
}
...
async fn a_manager_decides_their_branchs_people_and_not_another_branchs(pool: PgPool) {
    let app = app!(pool);
    let f = seed(&pool).await;
    let mine = file(&app, &f, f.e, json!({ "kind": "late_arrival", "on_date": "2026-09-10", "to_time": "10:00:00" })).await;
    let theirs = file(&app, &f, f.x, json!({ "kind": "late_arrival", "on_date": "2026-09-10", "to_time": "10:00:00" })).await;
    assert_eq!(mine["to_owner"], false);
    let (st, _) = decide(&app, &f.mgr_token(), &theirs["id"], json!({ "status": "approved" })).await;
    assert_eq!(st, 403, "B's request, A's manager");
    let (st, _) = decide(&app, &f.mgr_token(), &mine["id"], json!({ "status": "approved" })).await;
    assert_eq!(st, 200);
    ...
    let (st, _) = decide(&app, &f.mgr_token(), &other["id"], json!({ "status": "cancelled" })).await;
    assert_eq!(st, 400);
    let (st, _) = decide(&app, &f.mgr_token(), &other["id"], json!({ "status": "cancelled", "note": "asked by phone" })).await;
    assert_eq!(st, 200);
}
...
async fn an_excuse_is_paid_by_the_rule_of_its_branch_and_unpaid_time_is_deducted(pool: PgPool) {
    ...
    let ex = file(&app, &f, f.e, json!({ "kind": "excuse", "on_date": d, "from_time": "12:00:00", "to_time": "14:00:00" })).await;
    assert_eq!(ex["paid_default"], false, "the business rule is the default (RQ-7)");
    let (st, row) = decide(&app, &f.owner_token(), &ex["id"], json!({ "status": "approved" })).await;
    assert_eq!(st, 200);
    ...
    let (st, b) = send!(app, "PUT", "/staff/attendance/settings".to_string(), f.owner_token(), json!({ "branch_id": f.a, "excused_time_paid_default": true }));
    assert_eq!(st, 200, "{b}");
    let d2 = "2026-08-11";
    let rec2 = record(&pool, &f, f.e, f.day_shift, d2, (at(d2, "09:00"), at(d2, "17:00")), Some((at(d2, "09:00"), at(d2, "17:00"))), "present").await;
    let ex2 = file(&app, &f, f.e, json!({ "kind": "excuse", "on_date": d2, "from_time": "12:00:00", "to_time": "14:00:00" })).await;
    assert_eq!(ex2["paid_default"], true);
    let (_, row) = decide(&app, &f.owner_token(), &ex2["id"], json!({ "status": "approved" })).await;
    assert_eq!(row["is_paid"], true);
    assert_eq!(deduction(&pool, rec2, "excused_unpaid").await, 0, "paid time costs nothing");

    // An unpaid early departure: leaving at 15:00 of a 17:00 shift.
    let d3 = "2026-08-12";
    let rec3 = record(&pool, &f, f.e, f.day_shift, d3, (at(d3, "09:00"), at(d3, "17:00")), Some((at(d3, "09:00"), at(d3, "15:00"))), "present").await;
    let early = file(&app, &f, f.e, json!({ "kind": "early_departure", "on_date": d3, "from_time": "15:00:00" })).await;
    let (_, row) = decide(&app, &f.owner_token(), &early["id"], json!({ "status": "approved", "is_paid": false })).await;
    assert_eq!(row["is_paid"], false);
    assert_eq!(deduction(&pool, rec3, "excused_unpaid").await, 2_500, "two unpaid hours");
    let early_leave: i32 = sqlx::query_scalar("SELECT early_leave_minutes FROM attendance_records WHERE id = $1")
        .bind(rec3)
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(early_leave, 0, "leaving at the agreed time is not leaving early");
}
...
async fn a_first_half_leave_forgives_the_morning_and_prices_only_that_half(pool: PgPool) {
    ...
    let rec = record(&pool, &f, f.e, f.day_shift, d, (at(d, "09:00"), at(d, "17:00")), Some((at(d, "13:00"), at(d, "17:00"))), "late").await;
    rederive(&app, &f, rec).await;
    assert_eq!(deduction(&pool, rec, "late_penalty").await, DAY / 2, "without leave: 240 min late");

    let half = file(&app, &f, f.e, json!({ "kind": "leave", "on_date": d, "is_half_day": true, "leave_half": "first" })).await;
    assert_eq!(half["leave_half"], "first");
    let (st, _) = decide(&app, &f.owner_token(), &half["id"], json!({ "status": "approved", "is_paid": false })).await;
    assert_eq!(st, 200);
    let (status, late): (String, i32) = sqlx::query_as("SELECT status, late_minutes FROM attendance_records WHERE id = $1")
        .bind(rec)
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!((status.as_str(), late), ("present", 0), "the other half is worked normally");
    assert_eq!(deduction(&pool, rec, "late_penalty").await, 0);
    assert_eq!(deduction(&pool, rec, "absence").await, DAY / 2, "unpaid half day: half an absence");
}
...
async fn a_second_half_leave_lets_them_go_at_the_middle(pool: PgPool) {
    ...
    roster(&pool, &f, f.e, &[f.day_shift]).await;
    let d = "2026-08-10";
    let rec = record(&pool, &f, f.e, f.day_shift, d, (at(d, "09:00"), at(d, "17:00")), Some((at(d, "09:00"), at(d, "13:00"))), "present").await;
    let half = file(&app, &f, f.e, json!({ "kind": "leave", "on_date": d, "is_half_day": true, "leave_half": "second" })).await;
    let (st, _) = decide(&app, &f.owner_token(), &half["id"], json!({ "status": "approved", "is_paid": true })).await;
    assert_eq!(st, 200);
    let (status, early): (String, i32) = sqlx::query_as("SELECT status, early_leave_minutes FROM attendance_records WHERE id = $1")
        .bind(rec)
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!((status.as_str(), early), ("present", 0));
    assert_eq!(deduction(&pool, rec, "absence").await, 0, "paid half day costs nothing");
...
async fn under_the_whole_day_rule_a_half_day_is_a_day_off(pool: PgPool) {
    ...
    let d = "2026-08-10";
    let rec = record(&pool, &f, f.e, f.day_shift, d, (at(d, "09:00"), at(d, "17:00")), None, "absent").await;
    let half = file(&app, &f, f.e, json!({ "kind": "leave", "on_date": d, "is_half_day": true })).await;
    decide(&app, &f.owner_token(), &half["id"], json!({ "status": "approved", "is_paid": false })).await;
    let status: String = sqlx::query_scalar("SELECT status FROM attendance_records WHERE id = $1").bind(rec).fetch_one(&pool).await.unwrap();
    assert_eq!(status, "on_leave");
    assert_eq!(deduction(&pool, rec, "absence").await, DAY, "the whole day, unpaid");
}
...
async fn nobody_is_late_on_a_day_of_approved_leave(pool: PgPool) {
    ...
    roster(&pool, &f, f.e, &[f.day_shift]).await;
    let d = "2026-08-10";
    let rec = record(&pool, &f, f.e, f.day_shift, d, (at(d, "09:00"), at(d, "17:00")), Some((at(d, "11:00"), at(d, "12:00"))), "late").await;
    let leave = file(&app, &f, f.e, json!({ "kind": "leave", "on_date": d })).await;
    decide(&app, &f.owner_token(), &leave["id"], json!({ "status": "approved", "is_paid": true })).await;
    assert_eq!(deduction(&pool, rec, "late_penalty").await, 0, "B3: no late penalty on leave");
}
...
async fn a_missed_shift_of_a_split_day_costs_its_share(pool: PgPool) {
    ...
    let d = "2026-08-10";
    let am = record(&pool, &f, f.e, f.morning, d, (at(d, "09:00"), at(d, "13:00")), None, "absent").await;
    rederive(&app, &f, am).await;
    assert_eq!(deduction(&pool, am, "absence").await, DAY / 2, "half the day's rostered time");
    let pm = record(&pool, &f, f.e, f.evening, d, (at(d, "17:00"), at(d, "21:00")), None, "absent").await;
...
async fn a_late_arrival_for_the_evening_never_excuses_the_morning(pool: PgPool) {
    ...
    let am = record(&pool, &f, f.e, f.morning, d, (at(d, "09:00"), at(d, "13:00")), Some((at(d, "09:40"), at(d, "13:00"))), "late").await;
    let pm = record(&pool, &f, f.e, f.evening, d, (at(d, "17:00"), at(d, "21:00")), Some((at(d, "17:50"), at(d, "21:00"))), "late").await;
    let late = file(&app, &f, f.e, json!({ "kind": "late_arrival", "on_date": d, "to_time": "18:00:00" })).await;
    decide(&app, &f.owner_token(), &late["id"], json!({ "status": "approved" })).await;
    let lates: Vec<i32> = sqlx::query_scalar("SELECT late_minutes FROM attendance_records WHERE id = ANY($1) ORDER BY scheduled_start_at")
        ...
        .await
        .unwrap();
    assert_eq!(lates, vec![40, 0], "B4: the evening's permission stays with the evening");
    ...
    let d2 = "2026-08-11";
    let am2 = record(&pool, &f, f.e, f.morning, d2, (at(d2, "09:00"), at(d2, "13:00")), Some((at(d2, "09:40"), at(d2, "13:00"))), "late").await;
    let tied = file(&app, &f, f.e, json!({ "kind": "late_arrival", "on_date": d2, "to_time": "10:00:00", "work_shift_id": f.evening })).await;
    decide(&app, &f.owner_token(), &tied["id"], json!({ "status": "approved" })).await;
    let late2: i32 = sqlx::query_scalar("SELECT late_minutes FROM attendance_records WHERE id = $1").bind(am2).fetch_one(&pool).await.unwrap();
...
async fn a_night_shifts_times_after_midnight_land_on_the_next_morning(pool: PgPool) {
    ...
    let rec = record(&pool, &f, f.e, f.night, d, (at(d, "22:00"), at(next, "06:00")), Some((at(next, "00:30"), at(next, "06:00"))), "late").await;
    // A late arrival agreed for 00:30 means the next morning.
    let late = file(&app, &f, f.e, json!({ "kind": "late_arrival", "on_date": d, "to_time": "00:30:00" })).await;
    decide(&app, &f.owner_token(), &late["id"], json!({ "status": "approved" })).await;
    let lm: i32 = sqlx::query_scalar("SELECT late_minutes FROM attendance_records WHERE id = $1").bind(rec).fetch_one(&pool).await.unwrap();
    ...
    let phone = format!("{}|{}", s.token, s.device);
    let (st, corr) = send!(app, "POST", "/staff/me/requests".to_string(), phone, json!({ "kind": "correction", "on_date": d, "attendance_record_id": rec, "to_time": "05:00:00" }));
    assert_eq!(st, 201, "{corr}");
    assert_eq!(corr["record_check_out_at"].as_str().map(|s| s.starts_with("2026-08-11T06:00")), Some(true), "the approver sees the current punch: {corr}");
    let (st, b) = decide(&app, &f.owner_token(), &corr["id"], json!({ "status": "approved" })).await;
    assert_eq!(st, 200, "{b}");
    let out: DateTime<Utc> = sqlx::query_scalar("SELECT check_out_at FROM attendance_records WHERE id = $1").bind(rec).fetch_one(&pool).await.unwrap();
    assert_eq!(out, at(next, "05:00"));
    // An approved correction can't be cancelled: the punch is rewritten.
    let (st, _) = decide(&app, &f.owner_token(), &corr["id"], json!({ "status": "cancelled", "note": "n" })).await;
    assert_eq!(st, 409);
}
...
async fn a_request_decision_keeps_the_managers_status_and_audit(pool: PgPool) {
    ...
    roster(&pool, &f, f.e, &[f.day_shift]).await;
    let d = "2026-08-10";
    let rec = record(&pool, &f, f.e, f.day_shift, d, (at(d, "09:00"), at(d, "17:00")), Some((at(d, "09:40"), at(d, "17:00"))), "late").await;
    let (st, _) = send!(app, "PATCH", format!("/staff/attendance/{rec}"), f.owner_token(), json!({ "reason": "Card reader broke", "status": "present" }));
    assert_eq!(st, 200);
    let ex = file(&app, &f, f.e, json!({ "kind": "excuse", "on_date": d, "from_time": "12:00:00", "to_time": "13:00:00" })).await;
    decide(&app, &f.owner_token(), &ex["id"], json!({ "status": "approved", "is_paid": true })).await;
    ...
    .await
    .unwrap();
    assert_eq!(status, "present", "the manager's status survives the re-derive");
    assert_eq!(reason.as_deref(), Some("Card reader broke"), "B10: the Legal report keeps the manager's reason");
    assert!(overridden);
    // `derived` hands the day back to the rules.
    let (st, _) = send!(app, "PATCH", format!("/staff/attendance/{rec}"), f.owner_token(), json!({ "reason": "back to the rules", "status": "derived" }));
    assert_eq!(st, 200);
    let status: String = sqlx::query_scalar("SELECT status FROM attendance_records WHERE id = $1").bind(rec).fetch_one(&pool).await.unwrap();
...
async fn the_sweep_never_writes_back_a_day_a_manager_deleted(pool: PgPool) {
    ...
    let d = "2026-08-10";
    let rec = record(&pool, &f, f.e, f.day_shift, d, (at(d, "09:00"), at(d, "17:00")), None, "absent").await;
    let (st, _) = send!(app, "DELETE", format!("/staff/attendance/{rec}?reason=Was%20on%20a%20course"), f.owner_token());
    assert_eq!(st, 204);
    ...
    let reason: Option<String> = sqlx::query_scalar("SELECT reason FROM attendance_tombstones WHERE employee_id = $1").bind(f.e).fetch_one(&pool).await.unwrap();
    assert_eq!(reason.as_deref(), Some("Was on a course"));
    // A manager can still record the day by hand.
    let (st, b) = send!(app, "POST", "/staff/attendance".to_string(), f.owner_token(), json!({ "employee_id": f.e, "branch_id": f.a, "business_date": d, "work_shift_id": f.day_shift, "status": "present", "reason": "course counts as work" }));
    assert_eq!(st, 201, "{b}");
}
...
async fn a_waiver_can_be_taken_back_with_a_reason(pool: PgPool) {
    ...
    let rec = record(&pool, &f, f.e, f.day_shift, d, (at(d, "09:00"), at(d, "17:00")), None, "absent").await;
    rederive(&app, &f, rec).await;
    let id: Uuid = sqlx::query_scalar("SELECT id FROM payroll_deductions WHERE attendance_record_id = $1").bind(rec).fetch_one(&pool).await.unwrap();
    let (st, _) = send!(app, "PATCH", format!("/staff/payroll/deductions/{id}/waive"), f.owner_token(), json!({ "reason": "sick" }));
    assert_eq!(st, 200);
    let uri = format!("/staff/payroll/deductions/{id}/unwaive");
    // Refusals first (AT-11): no reason; a manager of another branch; an employee.
    let (st, _) = send!(app, "PATCH", uri.clone(), f.owner_token(), json!({ "reason": " " }));
    assert_eq!(st, 400);
    let other = user(&pool, f.org, "branch_manager").await;
    sqlx::query("INSERT INTO user_branch_assignments (user_id, branch_id) VALUES ($1, $2)").bind(other).bind(f.b).execute(&pool).await.unwrap();
    let (st, _) = send!(app, "PATCH", uri.clone(), user_token(other, f.org, UserRole::BranchManager), json!({ "reason": "r" }));
    ...
    assert_eq!((by, why.as_deref()), (Some(f.owner), Some("No sick note came")));
    let (st, _) = send!(app, "PATCH", uri, f.owner_token(), json!({ "reason": "again" }));
...
async fn a_branch_override_inherits_every_rule_it_does_not_set(pool: PgPool) {
    ...
    let (st, b) = send!(app, "PUT", "/staff/attendance/settings".to_string(), owner.clone(), json!({ "branch_id": f.a, "overtime_mode": "automatic" }));
    assert_eq!(st, 200, "{b}");
    assert_eq!(b["overtime_mode"], "automatic");
    assert_eq!(b["late_deduction_tiers"].as_array().map(Vec::len), Some(2), "B2: the business ladder, not an empty one");
    ...
    let (st, _) = send!(app, "PUT", "/staff/attendance/settings".to_string(), owner.clone(), json!({ "branch_id": f.a, "period_start_day": 1 }));
    assert_eq!(st, 400);
    let (st, _) = send!(app, "PUT", "/staff/attendance/settings".to_string(), owner.clone(), json!({ "inherit": ["overtime_mode"] }));
    assert_eq!(st, 400, "only a branch inherits");

    // Back to the business's value, field by field, and then altogether.
    let (st, b) = send!(app, "PUT", "/staff/attendance/settings".to_string(), owner.clone(), json!({ "branch_id": f.a, "inherit": ["overtime_mode"], "working_days_per_month": 26 }));
    assert_eq!(st, 200, "{b}");
    assert_eq!(b["overtime_mode"], "off");
    assert_eq!(b["overridden"], json!(["working_days_per_month"]));
    let list = body(call!(app, "GET", "/staff/attendance/settings/branches".to_string(), owner.clone())).await;
    assert_eq!(list.as_array().map(Vec::len), Some(2), "both branches for the owner: {list}");
    let (st, _) = send!(app, "DELETE", format!("/staff/attendance/settings/branches/{}", f.a), owner.clone());
    assert_eq!(st, 204);
...
async fn a_manager_sees_the_rules_of_their_branches_and_changes_none(pool: PgPool) {
    ...
    let f = seed(&pool).await;
    let mgr = f.mgr_token();
    let (st, b) = send!(app, "GET", "/staff/attendance/settings".to_string(), mgr.clone());
    assert_eq!(st, 200, "the business's rules: {b}");
    let (st, _) = send!(app, "GET", format!("/staff/attendance/settings?branch_id={}", f.a), mgr.clone());
    assert_eq!(st, 200, "their own branch");
    let (st, _) = send!(app, "GET", format!("/staff/attendance/settings?branch_id={}", f.b), mgr.clone());
    assert_eq!(st, 403, "another branch's rules");
    let list = body(call!(app, "GET", "/staff/attendance/settings/branches".to_string(), mgr.clone())).await;
    let ids: Vec<&str> = list.as_array().unwrap().iter().map(|r| r["branch_id"].as_str().unwrap()).collect();
    assert_eq!(ids, vec![f.a.to_string()], "only their branches' overrides");
    // Every write is refused.
    for (m, uri, b) in [
        ("PUT", "/staff/attendance/settings".to_string(), json!({ "absence_deduction_days": 3 })),
        ("PUT", "/staff/attendance/settings".to_string(), json!({ "branch_id": f.a, "absence_deduction_days": 3 })),
        ("PUT", "/staff/attendance/settings".to_string(), json!({ "branch_id": f.a, "inherit": ["absence_deduction_days"] })),
        ("DELETE", format!("/staff/attendance/settings/branches/{}", f.a), Value::Null),
    ] {
    ...
    let s = session(&pool, f.e).await;
    let (st, _) = send!(app, "GET", "/staff/attendance/settings".to_string(), format!("{}|{}", s.token, s.device));
    assert_eq!(st, 403);
    ...
    let rows: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM attendance_settings WHERE org_id = $1").bind(f.org).fetch_one(&pool).await.unwrap();
    assert_eq!(rows, 1);
}
...
async fn a_mission_needs_a_title_or_a_note(pool: PgPool) {
    let app = app!(pool);
    let f = seed(&pool).await;
    let (st, _) = send!(app, "POST", "/staff/requests".to_string(), f.owner_token(), json!({ "employee_id": f.e, "kind": "mission", "on_date": "2026-09-10", "reason": "  " }));
    assert_eq!(st, 400);
    let (st, row) = send!(app, "POST", "/staff/requests".to_string(), f.owner_token(), json!({ "employee_id": f.e, "kind": "mission", "on_date": "2026-09-10", "reason": "Bank" }));
    assert_eq!((st, row["title"].as_str()), (201, Some("Bank")));
    // A half day is the first or the second half, nothing else.
    let (st, _) = send!(app, "POST", "/staff/requests".to_string(), f.owner_token(), json!({ "employee_id": f.e, "kind": "leave", "on_date": "2026-09-12", "is_half_day": true, "leave_half": "middle" }));
    assert_eq!(st, 400);
    let (st, _) = send!(app, "POST", "/staff/requests".to_string(), f.owner_token(), json!({ "employee_id": f.e, "kind": "leave", "on_date": "2026-09-12", "leave_half": "first" }));
    assert_eq!(st, 400, "only a half day says which half");
    let (st, _) = send!(app, "POST", "/staff/requests".to_string(), f.owner_token(), json!({ "employee_id": f.e, "kind": "leave", "on_date": "2026-09-12", "work_shift_id": f.day_shift }));
    assert_eq!(st, 400, "a leave names no shift");
    let (st, _) = send!(app, "POST", "/staff/requests".to_string(), f.owner_token(), json!({ "employee_id": f.e, "kind": "late_arrival", "on_date": "2026-09-12", "to_time": "10:00:00", "work_shift_id": Uuid::new_v4() }));
...
fn the_half_of_a_split_day_is_its_first_shift() {
    ...
    let d = "2026-08-10";
    let split = [(at(d, "09:00"), at(d, "13:00")), (at(d, "17:00"), at(d, "21:00"))];
    assert_eq!(half_day_window(&split, "first"), Some((at(d, "09:00"), at(d, "13:00"))));
    assert_eq!(half_day_window(&split, "second"), Some((at(d, "13:00"), at(d, "21:00"))));
