...
fn enum_of(doc: &Value, schema: &str, field: &str) -> Vec<String> {
    ...
    let r = prop["$ref"]
        ...
        .or_else(|| {
            prop["oneOf"]
                .as_array()
                .and_then(|a| a.iter().find_map(|x| x["$ref"].as_str()))
        })
...
fn till_and_device_vocabularies_are_enums() {
    let doc = doc();
    assert_eq!(
        enum_of(&doc, "Till", "status"),
        ["open", "closed", "force_closed"]
    );
    assert_eq!(
        enum_of(&doc, "Till", "verification"),
        ["server", "lan", "unverified", "legacy"]
    );
    assert_eq!(
        enum_of(&doc, "TillBrief", "status"),
        ["open", "closed", "force_closed"]
    );
    assert_eq!(enum_of(&doc, "Device", "kind"), ["pos", "kds", "waiter"]);
    assert_eq!(
        enum_of(&doc, "RegisterDeviceRequest", "kind"),
        ["pos", "kds", "waiter"]
    );
}
