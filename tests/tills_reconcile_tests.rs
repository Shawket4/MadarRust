...
fn reconciliation_status_rollup() {
    ...
    assert_eq!(
        rollup_status(["unreviewed", "disagreed", "checked"]),
        "disagreed"
    );
}
...
fn plan_live_disagreed_needs_amount_and_note() {
    ...
    let e = plan_lines(
        ...
        500,
        None,
        &[input("card", "disagreed", None, Some("x"))],
        false,
    )
    .unwrap_err();
    ...
    let e = plan_lines(
        ...
        500,
        None,
        &[input("card", "disagreed", Some(800), Some("  "))],
        false,
    )
    .unwrap_err();
...
fn plan_unlisted_unreviewed_and_unused_input_stored_zero() {
    ...
    let lines = plan_lines(
        ...
        Some("fine"),
        &[
            input("card", "checked", Some(1), None),
            input("instapay", "checked", None, None),
...
pub(crate) async fn fixture(pool: &PgPool) -> Fx {
    ...
    sqlx::query(
        "INSERT INTO tills (id, branch_id, teller_id, opening_cash) VALUES ($1, $2, $3, 100)",
    )
    .bind(till)
    .bind(branch)
    .bind(teller)
...
pub(crate) async fn order(
    ...
    fx: &Fx,
    legs: &[(&str, i32, bool)],
    tip: Option<(&str, i32, bool)>,
) -> Uuid {
    let id = Uuid::new_v4();
    let total: i32 = legs.iter().map(|l| l.1).sum();
    let pm = if legs.len() == 1 {
        legs[0].0.to_string()
    } else {
        "mixed".into()
    };
...
async fn totals(pool: &PgPool, till: Uuid, expected: i64) -> Vec<MethodTotal> {
    ...
    system_totals_by_method(&mut c, till, expected)
        .await
        .unwrap()
}
...
async fn system_totals_by_method_matches_report_payment_summary(pool: PgPool) {
    ...
    let non_cash: Vec<(String, i64, i64)> = t
        .iter()
        .filter(|m| !m.is_cash)
        .map(|m| (m.method.clone(), m.system_total, m.order_count))
        .collect();
    assert_eq!(non_cash, summary);
    assert_eq!(non_cash[0], ("card".into(), 1000, 2));
    assert!(
        t.iter()
            .find(|m| m.method == "card")
            .unwrap()
            .payment_method_id
            .is_some()
    );
...
async fn cash_row_uses_expected_cash(pool: PgPool) {
    ...
    assert_eq!(
        t.iter().find(|m| m.method == "card").unwrap().system_total,
        30
    );
}
...
async fn close_writes_checked_disagreed_unreviewed(pool: PgPool) {
    ...
    let bad = write_close_reconciliation(
        &mut tx,
        fx.till,
        fx.teller,
        150,
        150,
        None,
        &[input("card", "disagreed", Some(650), None)],
        false,
    )
    .await
    .unwrap_err();
    ...
    let (lines, status) = write_close_reconciliation(
        &mut tx,
        fx.till,
        fx.teller,
        140,
    ...
    assert_eq!(
        (
            by("cash").status.as_str(),
            by("cash").declared_amount,
            by("cash").system_total
        ),
        ("disagreed", Some(140), 150)
    );
    assert_eq!(
        (by("card").status.as_str(), by("card").declared_amount),
        ("disagreed", Some(650))
    );
    assert_eq!(by("wallet").status, "unreviewed");
    assert!(
        lines
            .iter()
            .all(|l| !l.changed_after_close && l.reconciled_by == Some(fx.teller))
    );
    ...
    let mut c = pool.acquire().await.unwrap();
    let (again, st2) = write_close_reconciliation(
        &mut c,
        fx.till,
        fx.teller,
        999,
...
async fn close_all_checked_is_clean_and_replay_stores_no_note(pool: PgPool) {
    ...
    close_status(&pool, fx.till).await;
    let mut c = pool.acquire().await.unwrap();
    let (_, s) = write_close_reconciliation(
        &mut c,
        fx.till,
        fx.teller,
        100,
    ...
    let (lines, s) = write_close_reconciliation(
        &mut c,
        fx2.till,
        fx2.teller,
        100,
        100,
        None,
        &[input("card", "disagreed", Some(600), None)],
        true,
    )
    .await
    .unwrap();
...
async fn late_replay_updates_current_total_only(pool: PgPool) {
    ...
    close_status(&pool, fx.till).await;
    let mut c = pool.acquire().await.unwrap();
    let (before, s) = write_close_reconciliation(
        &mut c,
        fx.till,
        fx.teller,
        100,
    ...
    recompute_after_late_replay(&mut c, fx.till, 130)
        .await
        .unwrap();
    recompute_after_late_replay(&mut c, fx.till, 130)
        .await
    ...
    assert_eq!(
        (
            by("card").system_total,
            by("card").current_system_total,
            by("card").status.as_str()
        ),
        (700, 750, "checked")
    );
    assert!(by("card").changed_after_close);
    assert_eq!(
        (by("cash").system_total, by("cash").current_system_total),
        (100, 130)
    );
    assert_eq!(
        (
            by("wallet").system_total,
            by("wallet").current_system_total,
            by("wallet").status.as_str()
        ),
        (0, 20, "unreviewed")
    );
    assert_eq!(before.len() + 1, after.len());
    let rs: Option<String> =
        sqlx::query_scalar("SELECT reconciliation_status FROM tills WHERE id = $1")
            .bind(fx.till)
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    let fx2 = fixture(&pool).await;
    close_status(&pool, fx2.till).await;
    recompute_after_late_replay(&mut c, fx2.till, 0)
        .await
        .unwrap();
