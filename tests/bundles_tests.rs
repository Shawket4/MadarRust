...
use madar_rust::bundles::handlers::*;
use madar_rust::bundles::routes;
...
async fn link_recipe(pool: &PgPool, item_id: Uuid, ing_id: Uuid, qty: f64) {
    sqlx::query("INSERT INTO menu_item_recipes (menu_item_id, org_ingredient_id, ingredient_name, ingredient_unit, quantity_used, size_label) VALUES ($1, $2, 'Ing', 'g'::inventory_unit, $3, 'one_size')")
        .bind(item_id)
        .bind(ing_id)
        .bind(qty)
        .execute(pool)
...
async fn test_bundles_crud(pool: PgPool) {
    ...
    let req2 = test::TestRequest::get()
        .uri(&format!("/bundles/{}", bundle.bundle.id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .to_request();
    ...
    let req3 = test::TestRequest::get()
        .uri(&format!("/bundles?org_id={}", org_id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .to_request();
    ...
    // Update Bundle
    let req4 = test::TestRequest::patch()
        .uri(&format!("/bundles/{}", bundle.bundle.id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
    ...
    // Delete Bundle
    let req5 = test::TestRequest::delete()
        .uri(&format!("/bundles/{}", bundle.bundle.id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .to_request();
...
async fn test_bundle_activation_and_rules(pool: PgPool) {
    ...
    let req4 = test::TestRequest::post()
        .uri(&format!("/bundles/{}/activate", cheap_bundle.bundle.id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .to_request();
    let resp4 = test::call_service(&app, req4).await;
    ...
    let req6 = test::TestRequest::post()
        .uri(&format!("/bundles/{}/activate", exp_bundle.bundle.id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .to_request();
...
async fn test_bundle_activation_blocked_on_unknown_component_cost(pool: PgPool) {
    ...
    sqlx::query("INSERT INTO menu_item_recipes (menu_item_id, org_ingredient_id, ingredient_name, ingredient_unit, quantity_used, size_label) VALUES ($1, NULL, 'Spice', 'g', 5.0, 'one_size')")
    ...
    assert_eq!(
        resp.status(),
        400,
        "activation must be blocked on unknown component cost"
    );
}
...
async fn test_bundle_performance_uses_snapshot_cost(pool: PgPool) {
    ...
    sqlx::query("INSERT INTO branches (id, org_id, name) VALUES ($1,$2,'Br')")
        .bind(branch_id)
        .bind(org_id)
        .execute(&pool)
        .await
    ...
    sqlx::query("INSERT INTO orders (id, branch_id, teller_id, till_id, idempotency_key, subtotal, tax_amount, total_amount, status, order_number, payment_method, order_ref) VALUES ($1,$2,$3,$4, gen_random_uuid(), 300, 0, 300, 'completed', 1, 'cash', gen_random_uuid()::text)")
    ...
    sqlx::query("UPDATE org_ingredients SET cost_per_unit = 500 WHERE id = $1")
        .bind(ing)
        .execute(&pool)
        .await
        .unwrap();
    ...
    assert_eq!(
        perf.net_profit, 200,
        "net_profit must be 300 - sale-time 100, not affected by current cost"
    );
}
