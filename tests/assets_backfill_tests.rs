...
async fn fixture(pool: &PgPool) -> Fx {
    ...
    let missing = seed_item(
        pool,
        org,
        Some(&format!("{base}/{org}/menu-items/gone.png")),
    )
    .await;
...
fn opts(fx: &Fx, dry: bool) -> BackfillOptions {
    BackfillOptions {
        ...
        step_animations_dir: None,
    }
}

async fn counts(pool: &PgPool) -> (i64, i64, i64) {
    let a: i64 = sqlx::query_scalar("SELECT count(*) FROM assets")
        .fetch_one(pool)
        .await
        .unwrap();
    let b: i64 = sqlx::query_scalar("SELECT count(*) FROM asset_backfill_items")
        .fetch_one(pool)
        .await
        .unwrap();
    let c: i64 =
        sqlx::query_scalar("SELECT count(*) FROM menu_items WHERE image_group_id IS NOT NULL")
            .fetch_one(pool)
            .await
            .unwrap();
    (a, b, c)
...
async fn status(pool: &PgPool, id: Uuid) -> String {
    sqlx::query_scalar("SELECT status FROM asset_backfill_items WHERE source_id=$1")
        .bind(id.to_string())
        .fetch_one(pool)
        .await
        .unwrap()
...
async fn backfill_marks_missing_and_broken_and_rerun_is_noop(pool: PgPool) {
    ...
    let g: Vec<Option<Uuid>> =
        sqlx::query_scalar("SELECT image_group_id FROM menu_items WHERE id = ANY($1) ORDER BY id")
            .bind(vec![fx.good, fx.dup])
            .fetch_all(&pool)
            .await
            .unwrap();
    assert!(g[0].is_some() && g[0] == g[1], "deduped to one group");
    // legacy URLs untouched, legacy paths mapped
    let url: String = sqlx::query_scalar("SELECT image_url FROM menu_items WHERE id=$1")
        .bind(fx.good)
        .fetch_one(&pool)
        .await
        .unwrap();
    assert!(url.ends_with("/a.png"));
    let mapped: i64 = sqlx::query_scalar("SELECT count(*) FROM asset_legacy_paths")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(mapped, 2);
    ...
    let updated: Vec<chrono::DateTime<chrono::Utc>> =
        sqlx::query_scalar("SELECT updated_at FROM asset_backfill_items ORDER BY source_id")
            .fetch_all(&pool)
            .await
            .unwrap();
    ...
    let updated2: Vec<chrono::DateTime<chrono::Utc>> = sqlx::query_scalar(
        "SELECT updated_at FROM asset_backfill_items WHERE status='verified' ORDER BY source_id",
    )
    .fetch_all(&pool)
    .await
    .unwrap();
    assert!(
        updated2.iter().all(|u| updated.contains(u)),
        "verified items not touched again"
    );
}
...
async fn backfill_resumes_after_kill(pool: PgPool) {
    ...
    let pending: i64 =
        sqlx::query_scalar("SELECT count(*) FROM asset_backfill_items WHERE status='pending'")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(pending, 3);
    // simulate a crash between INGEST and VERIFY
    sqlx::query("UPDATE asset_backfill_items SET status='ingested' WHERE status='verified'")
        .execute(&pool)
        .await
        .unwrap();
    let rep = run(&pool, &opts(&fx, false)).await.unwrap();
    assert_eq!(
        (rep.totals.verified, rep.totals.missing, rep.totals.broken),
        (2, 1, 1)
    );
    let groups: i64 = sqlx::query_scalar("SELECT count(*) FROM asset_groups")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(groups, 1);
...
async fn backfill_prune_requires_verified_run(pool: PgPool) {
    ...
    assert!(
        prune(&pool, &fx.store, Some(fx.org), unknown, false)
            .await
            .is_err()
    );
    let o = opts(&fx, false);
    ...
    assert!(
        prune(&pool, &fx.store, Some(fx.org), o.run_id, false)
            .await
            .is_err()
    );
    let a = fx
        .store
        .uploads_dir
        .join(format!("{}/menu-items/a.png", fx.org));
    assert!(a.exists());
    let rep = prune(&pool, &fx.store, Some(fx.org), o.run_id, true)
        .await
        .unwrap();
    ...
    assert!(
        fx.store
            .uploads_dir
            .join(format!("{}/menu-items/bad.png", fx.org))
            .exists(),
        "unverified originals kept"
    );
}
...
async fn encoder_version_bump_creates_new_group_on_backfill(pool: PgPool) {
    ...
    sqlx::query("UPDATE asset_groups SET encoder = 'madar-ingest/0 old'")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query(
        "UPDATE assets SET encoder = 'madar-ingest/0 old', hash = md5(hash) || md5(hash || 'x')",
    )
    .execute(&pool)
    .await
    .unwrap();
    sqlx::query("UPDATE asset_backfill_items SET status='pending' WHERE status='verified'")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("UPDATE menu_items SET image_group_id = NULL")
        .execute(&pool)
        .await
        .unwrap();
    run(&pool, &opts(&fx, false)).await.unwrap();
...
async fn files_vs_rows(pool: &PgPool, store: &AssetStore, org: Uuid) -> (Vec<String>, Vec<String>) {
    ...
    let rows: Vec<String> = sqlx::query_scalar(
        "SELECT DISTINCT hash || '.' || ext FROM assets WHERE org_id = $1 ORDER BY 1",
    )
    .bind(org)
    .fetch_all(pool)
    .await
...
async fn variants_of(pool: &PgPool, g: Uuid) -> Vec<String> {
    sqlx::query_scalar("SELECT variant FROM assets WHERE group_id = $1 ORDER BY variant")
        .bind(g)
        .fetch_all(pool)
        .await
        .unwrap()
...
async fn backfill_logo_with_same_pixels_as_menu_photo_but_other_bytes(pool: PgPool) {
    ...
    assert_eq!(
        image::load_from_memory(&photo).unwrap().to_rgba8(),
        image::load_from_memory(&logo).unwrap().to_rgba8()
    );
    ...
    set_org_images(
        &pool,
        org,
        Some(&format!("{base}/logos/l.png")),
        Some(&format!("{base}/card/c.png")),
    )
    .await;
    ...
    let pg = group_of(
        &pool,
        "SELECT image_group_id FROM menu_items WHERE id=$1",
        item,
    )
    .await;
    let lg = group_of(
        &pool,
        "SELECT logo_group_id FROM organizations WHERE id=$1",
        org,
    )
    .await;
    let cg = group_of(
        &pool,
        "SELECT brand_card_image_group_id FROM organizations WHERE id=$1",
        org,
    )
    .await;
    ...
    assert!(
        shared >= 1,
        "the colliding variant is stored once and referenced twice"
    );
    ...
    let on_disk: i64 = disk
        .iter()
        .map(|f| {
            std::fs::metadata(store.assets_dir.join(org.to_string()).join(f))
                .unwrap()
                .len() as i64
        })
        .sum();
...
async fn backfill_same_file_as_menu_photo_and_logo_gets_two_profiles(pool: PgPool) {
    ...
    let item = seed_item(
        &pool,
        org,
        Some(&format!("{base}/{org}/menu-items/same.png")),
    )
    .await;
    set_org_images(
        &pool,
        org,
        Some(&format!("{base}/logos/same.png")),
        Some(&format!("{base}/card/same.png")),
    )
    .await;

    let rep = run(&pool, &opts_org(&store, org)).await.unwrap();
    assert_eq!(
        (rep.totals.verified, rep.totals.failed),
        (3, 0),
        "{:?}",
        rep.items_failed
    );
    ...
    for g in [lg, cg] {
        assert!(
            variants_of(&pool, g)
                .await
                .contains(&"original".to_string()),
            "logo/card keep an original"
        );
    }
    let profiles: Vec<(Uuid, String)> =
        sqlx::query_as("SELECT id, profile FROM asset_groups WHERE org_id=$1")
            .bind(org)
            .fetch_all(&pool)
            .await
            .unwrap();
    ...
    assert!(
        profiles.contains(&(pg, "photo".into()))
            && profiles.contains(&(lg, "keeps_original".into()))
    );
    let (disk, rows) = files_vs_rows(&pool, &store, org).await;
    assert_eq!(disk, rows);
}
...
async fn ingest_different_images_with_byte_identical_thumbnails(pool: PgPool) {
    ...
    assert!(!b.deduped);
    assert_eq!(
        a.variant("thumb").unwrap().hash,
        b.variant("thumb").unwrap().hash,
        "precondition: identical thumbnails"
    );
    assert_ne!(
        a.variant("full").unwrap().hash,
        b.variant("full").unwrap().hash,
        "precondition: different images"
    );
...
async fn failed_ingest_attempt_leaves_no_orphan_and_keeps_shared_files(pool: PgPool) {
    ...
    for _ in 0..3 {
        ...
        assert_eq!(
            disk, before,
            "the attempt's own files are gone, the shared ones stay"
        );
        assert_eq!(disk, rows);
    }
...
async fn backfill_with_shared_files_rerun_is_noop(pool: PgPool) {
    ...
    let rep = run(&pool, &opts_org(&store, org)).await.unwrap();
    assert_eq!(
        (rep.totals.verified, rep.totals.failed, rep.totals.deduped),
        (4, 0, 2),
        "{:?}",
        rep.items_failed
    );
    ...
    let snapshot = |pool: PgPool| async move {
        ...
        let assets: Vec<(Uuid, String, String)> =
            sqlx::query_as("SELECT group_id, variant, hash FROM assets ORDER BY 1, 2")
                .fetch_all(&pool)
                .await
                .unwrap();
        (rows, assets)
    ...
    let first_id = sqlx::query_scalar::<_, Uuid>(
        "SELECT DISTINCT run_id FROM asset_backfill_items WHERE status='verified'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
...
async fn prune_keeps_a_legacy_file_another_unverified_item_uses(pool: PgPool) {
    ...
    run(&pool, &o).await.unwrap();
    sqlx::query(
        "UPDATE asset_backfill_items SET status='failed' WHERE source_table='organizations'",
    )
    .execute(&pool)
    .await
    .unwrap();
    let rep = prune(&pool, &store, Some(org), o.run_id, true)
        .await
        .unwrap();
    ...
    assert!(
        store
            .uploads_dir
            .join(format!("{org}/menu-items/s.png"))
            .exists()
    );
}
...
async fn small_transparent_logo_keeps_an_original_row_sharing_the_full_file(pool: PgPool) {
    ...
    for x in 50..150 {
        for y in 25..75 {
            img.put_pixel(x, y, image::Rgba([10, 120, 60, 255]));
        }
    }
    let mut buf = std::io::Cursor::new(Vec::new());
    image::DynamicImage::ImageRgba8(img)
    ...
    .unwrap();
    let (full, orig) = (
        o.variant("full").unwrap(),
        o.variant("original").expect("original row"),
    );
    assert_eq!(
        full.hash, orig.hash,
        "precondition: lossless full == original"
    );
