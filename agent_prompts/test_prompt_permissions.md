...
# Task
Your task is to write exhaustive tests for the `permissions` module in the `SufrixRust` project.

# Context to Gather First
Before writing any test code, you MUST use your file reading tools to deeply understand the module. Please read:
1. `src/permissions/handlers.rs` (the controller logic and the SQL queries being executed)
2. `src/permissions/routes.rs` (the Actix route configurations)
3. `src/permissions/mod.rs` (domain models and specific structs)
...
# Execution Loop
1. Create `src/permissions/tests.rs` containing your test suite.
2. Update `src/permissions/mod.rs` to include `#[cfg(test)] mod tests;`.
3. Run `cargo test permissions`.
