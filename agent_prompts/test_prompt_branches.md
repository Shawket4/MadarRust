...
# Task
Your task is to write exhaustive tests for the `branches` module in the `SufrixRust` project.

# Context to Gather First
Before writing any test code, you MUST use your file reading tools to deeply understand the module. Please read:
1. `src/branches/handlers.rs` (the controller logic and the SQL queries being executed)
2. `src/branches/routes.rs` (the Actix route configurations)
3. `src/branches/mod.rs` (domain models and specific structs)
...
# Execution Loop
1. Create `src/branches/tests.rs` containing your test suite.
2. Update `src/branches/mod.rs` to include `#[cfg(test)] mod tests;`.
3. Run `cargo test branches`.
