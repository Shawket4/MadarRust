...
| `LOYALTY_PROXIMITY_METERS` | How near a branch a customer must be for their card to surface on the lock screen. Optional; defaults to **500**. Widen it for a unit you approach across a car park, tighten it for a kiosk on a busy street — a card that surfaces while someone drives past is noise, and noise gets passes deleted. | Cards surface at 500 m. |
