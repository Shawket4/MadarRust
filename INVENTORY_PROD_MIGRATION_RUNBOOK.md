...
> data-preserving** and was validated on `madar_dev` (a fresh prod-data copy) —
...
|---|---|---|
| **dev** (`localhost:5433/madar_dev`) | `20260613010000` | fully migrated + recipe-unit backfill applied; tested |
| **prod** (`100.101.100.57:5432/madar`) | `20260612130000` | **missing the 8 migrations below** |
...
2. **Back up first** (mandatory):
   ...
   /opt/homebrew/opt/postgresql@17/bin/pg_dump \
     'postgres://madar:<PWD>@100.101.100.57:5432/madar' \
     --no-owner --no-privileges -Fc -f madar_prod_pre_inventory_$(date +%Y%m%d_%H%M).dump
   ```
...
4. **Backend + clients:** deploy the new backend binary built from this repo **with** the
   ...
   contract; regenerate the dashboard Orval client and the POS `madar_api` package from the new
...
2. Regenerate API clients from the new `openapi.json` (702 KB, in this repo):
   - Dashboard: `npm run generate:api` (Orval) — see `MadarDashboard/INVENTORY_FRONTEND_HANDOFF.md`.
   - POS: regenerate the `madar_api` dart package.
